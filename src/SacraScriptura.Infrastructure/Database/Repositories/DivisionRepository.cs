...
public class DivisionRepository : IDivisionRepository
{
    ...
    {
        var parent = await _context.Divisions.FindAsync(parentId);
        if (parent == null)
        {
    ...
    public async Task AddAsync(Division division)
    {
        ...
        var anyDivisions = await _context.Divisions
        ...
        else
        {
            ...
            var maxRightValue = await _context.Divisions
                .Where(d => d.BookId == division.BookId && d.Depth == 0)
                .MaxAsync(d => d.RightValue);

            var maxOrder = await _context.Divisions
                .Where(d => d.BookId == division.BookId && d.Depth == 0)
                .MaxAsync(d => d.Order);
    ...
    {
        var sibling = await _context.Divisions.FindAsync(siblingId)
    ...
    public async Task MoveToChildOfAsync(DivisionId divisionId, DivisionId newParentId)
    {
        ...
        var newParent = await _context.Divisions.FindAsync(newParentId)
        ...
        var subtreeNodes = await _context.Divisions
            .Where(d => d.BookId == division.BookId &&
                        d.LeftValue >= division.LeftValue &&
                        d.RightValue <= division.RightValue)
            .ToListAsync();
    ...
    public async Task MoveBeforeAsync(DivisionId divisionId, DivisionId siblingId)
    {
        ...
        if (division.ParentId != sibling.ParentId)
        {
            ...
            division = await _context.Divisions.FindAsync(divisionId);
        }
